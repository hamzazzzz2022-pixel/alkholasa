...
export default function DashboardPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-950 dark:bg-slate-950 bg-slate-50 text-slate-900 dark:text-white flex items-center justify-center dir-rtl">
        ...
        <div className="space-y-4">
          ...
          {courses.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {courses.map((course) => (
                <div key={course.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl space-y-3 shadow-md flex flex-col justify-between transition-colors duration-200">
